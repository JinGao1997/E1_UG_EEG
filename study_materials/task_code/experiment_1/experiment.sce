...
trial {trial_duration=3000;
       trial_type=specific_response;
		 terminator_button = 1,2; 
	  	 stimulus_event{
			picture {text{caption="o"; preload=false; font_size=20;}oben;  x=0; y= 20;
						text{caption="u"; preload=false; font_size=20;}unten; x=0; y=-20;
						text{caption="ar links";  preload=false; font_size=16;}ar_l; x=-62; y=-80;
						text{caption="ar rechts"; preload=false; font_size=16;}ar_r; x= 62; y=-80;
			}fb;
		 response_active=true;}feedbacktrigger;
		}feedback;
...
output_port out = output_port_manager.get_port(1);
...
file.print("subject\tsetting\tblock\tindex\tstim\tstim_trigger\tOffers_Other\tOffers_You\tOffer_Trigger\treaction\tRT\n");
...
begin
	...
	key=0; RT=0;
	if start==1 || start==181 || start==361 then AcRe=1 else AcRe=2 end; ### blockweise wechseln 
	...
	YouOther=random(1,2);
	if YouOther==1 then oben.set_caption ("          Sie -&gt; <b>"+stim[i][4]+"0</b> ct"); unten.set_caption("Partner/in -&gt; <b>"+stim[i][3]+"0</b> ct")
						else oben.set_caption ("Partner/in -&gt; <b>"+stim[i][3]+"0</b> ct");   unten.set_caption("          Sie -&gt; <b>"+stim[i][4]+"0</b> ct") end;
	...
	pictrigger.set_port_code(int(stim[i][2]));
	feedbacktrigger.set_port_code(int(stim[i][5])); ### offerTrigger
...
feedback.present();
   ...
   response_manager.set_button_codes({0,0,0,0}); ### Tasten-Trigger
	...
	if 	 reaction==1 && YouOther==1 then oben2.set_caption ("      Sie erhalten <b>"+stim[i][4]+"0</b> ct"); unten2.set_caption("Partner/in erhält <b>"+stim[i][3]+"0</b> ct");
	elseif reaction==1 && YouOther==2 then oben2.set_caption ("Partner/in erhält <b>"+stim[i][3]+"0</b> ct");  unten2.set_caption("      Sie erhalten <b>"+stim[i][4]+"0</b> ct"); 
	elseif reaction==2 && YouOther==1 then oben2.set_caption ("      Sie erhalten <b>0</b> ct"); unten2.set_caption("Partner/in erhält <b>0</b> ct");	
	elseif reaction==2 && YouOther==2 then oben2.set_caption ("Partner/in erhält <b>0</b> ct");  unten2.set_caption("      Sie erhalten <b>0</b> ct");
	end;	
	oben2.load(); unten2.load();

	if 	 stim[i][3]=="5" then feedbacktrigger2.set_port_code(3)
	elseif stim[i][3]=="6" then feedbacktrigger2.set_port_code(4)
	elseif stim[i][3]=="7" then feedbacktrigger2.set_port_code(5)
	elseif stim[i][3]=="8" then feedbacktrigger2.set_port_code(6)
	elseif stim[i][3]=="9" then feedbacktrigger2.set_port_code(7)
	end;

wait_interval(100); ### triggerbreak

if key==0 then out.send_code(8); wait_interval(100);
 missed.present() else feedback2.present() end;
...
if prac==0 then logfile() end;
	...
	if i%30==0 && i!=ende then instruktion("Folie7.PNG") end;
...
instruktion("Folie3.PNG"); block=1; main(  1, 90); instruktion("Folie8.PNG");
instruktion("Folie4.PNG"); block=2; main( 91,180); instruktion("Folie8.PNG");
instruktion("Folie3.PNG"); block=3; main(181,270); instruktion("Folie8.PNG");
instruktion("Folie4.PNG"); block=4; main(271,360); instruktion("Folie8.PNG");
instruktion("Folie3.PNG"); block=5; main(361,450); instruktion("Folie8.PNG");
instruktion("Folie4.PNG"); block=6; main(451,540);
