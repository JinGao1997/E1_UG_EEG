...
"""
Experiment 1 Master Analysis Script: Stimulus-Locked Components (FRN, LPP, N400)
=============================================================================
Description:
    The complete analysis and visualization pipeline for Experiment 1 (E1).
    
    EXPERIMENTAL DESIGN:
    - Sequential Presentation: Face (-1000ms) -> Offer (0ms) -> Response.
    - Implication: The pre-offer baseline (-200ms to 0ms) is potentially 
      modulated by the preceding emotional face.

    STATISTICAL MODEL (REGRESSION METHOD):
    - Model: Signal ~ Intercept + Emotion + Offer + Emo*Offer + Baseline + Emo*Baseline
    - Interaction Term: The 'Emotion * Baseline' interaction is INCLUDED.
      Rationale: Unlike Exp 2, the baseline slope in Exp 1 may vary by emotion 
      due to the sustained potential from the preceding face. Including this 
      interaction prevents underfitting and ensures accurate isolation of the 
      phasic offer response.

    MODULES:
    1. RegressionAnalysis: LMM-corrected waveforms (Interaction Model).
...
def get_paths():
    ...
    # Define E1 specific output directory to avoid overwriting E2 results
    out_dir_base = script_path.parent / f"Results_{TARGET_COMPONENT}_E1" / f"Session_{ts}"
...
class VisualizationEngine:
    ...
    def plot_unified_panels(data_dict, time_ms, title_suffix, save_path):
        ...
        for idx, offer in enumerate(OFFER_TYPES):
            ...
            clean_suffix = title_suffix.replace(" (Exp 1 Model)", "").replace(" (Interaction)", "")
...
# ==============================================================================
# 9. REGRESSION ANALYSIS MODULE (EXP 1 INTERACTION MODEL)
# ==============================================================================
...
class RegressionAnalysis:
    ...
    CRITICAL CHANGE:
    Includes 'Emo*Baseline' interaction term. This accounts for the sequential 
    design where pre-offer baseline is modulated by the preceding face emotion.
    """
    def __init__(self):
    ...
    def run(self):
        print(f"\n🚀 Regression: {TARGET_COMPONENT} (Exp 1 - Interaction Model)")
        ...
        self.viz.plot_unified_panels(
            ...
            "Regression-Corrected (Exp 1 Model)", self.out/f"{TARGET_COMPONENT}_Regression_Unified.tif"
        )
    ...
    def _process_topomaps(self, df, channels):
        ...
        # 5. Baseline Main Effect
        X_list.append(dt['BL'].values) 
        
        # 6. [NEW] Emotion * Baseline Interaction (Exp 1 Specific)
        for e in ['aff','dis','dom','enj']: X_list.append(dt[f'e_{e}'].values * dt['BL'].values)
        ...
        for ch in tqdm(channels, desc="   Topomaps"):
            ...
            try:
                ...
                # Reconstruction: Eval at BL=0 (Interaction terms also vanish)
                for ofr in OFFER_TYPES:
                    ...
                    for emo in COLORS:
                        ...
                        # Note: b[10] (BL Main) and b[11-14] (Emo*BL) are multiplied by 0
    ...
    def _perform_regression_roi(self, df, roi_chs):
        ...
        for t in tqdm(times, desc="   Fitting"):
            ...
            # [EXP 1 MODEL CONSTRUCTION - SAME AS ABOVE]
            X_list = [np.ones(len(dt))]
            for e in ['aff','dis','dom','enj']: X_list.append(dt[f'e_{e}'].values)
            X_list.append(dt['oc'].values)
            for e in ['aff','dis','dom','enj']: X_list.append(dt[f'e_{e}'].values * dt['oc'].values)
            
            X_list.append(dt['BL'].values) # Baseline Main
            
            # [NEW] Emo * Baseline Interaction
            for e in ['aff','dis','dom','enj']: X_list.append(dt[f'e_{e}'].values * dt['BL'].values)
...
if __name__ == "__main__":
    ...
    for comp in BATCH_COMPONENTS:
        ...
        if ANALYSIS_METHOD == 'All':
            # 1. Regression Method (Corrected via LMM Interaction Model)
