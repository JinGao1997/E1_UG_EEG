...
"""
Master Analysis & Visualization Script: Response-Locked Component (CPP)
=============================================================================
Description:
    Unified visualization pipeline for both Experiment 1 (E1) and Experiment 2 (E2)
    specifically for the Response-Locked CPP component.
    
    STATISTICAL MODEL DYNAMICS:
    - [E1 & E2] Model: Signal ~ Intercept + Emotion + Offer + Emo*Offer + Baseline
    
    [METHODOLOGICAL NOTE]: 
    Baseline ('BL') is strictly mean-centered before OLS fitting. This enforces 
    the ANCOVA assumption of homogeneity of regression slopes. When reconstructing 
    the marginal means (setting BL=0 in the equation), the waveforms reflect the 
    amplitude at the *average* baseline level, not at an extrapolated absolute 0 µV.

    [COVARIATE SOURCE]:
    - Both models borrow 'Baseline_LPP_offer' from the Stimulus-Locked output 
      as the pre-stimulus neural state covariate.
...
# ==============================================================================
# 1. GLOBAL EXPERIMENT TOGGLE
# ==============================================================================
# Switch between 'E1' and 'E2' to dynamically adjust path mapping
...
def get_paths():
    """
    Robustly detects the project root and sets paths based on EXPERIMENT_VERSION.
    """
    ...
    for parent in [script_path.parent] + list(script_path.parents)[:5]:
        if (parent / "data" / f"02_Pipeline_Output_{EXPERIMENT_VERSION}").exists():
            root = parent
            break
        # Fallback check for old structure just in case
        elif (parent / "data" / "02_Pipeline_Output").exists():
            root = parent
    ...
    # Dynamic Pipeline Output Path based on Experiment Version
    pipe_out = root / "data" / f"02_Pipeline_Output_{EXPERIMENT_VERSION}"
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    paths = {
        # OUTPUT: Dynamically marked with _E1 or _E2
        "results": script_path.parent / f"Results_{TARGET_COMPONENT}_{EXPERIMENT_VERSION}" / f"Session_{ts}",
        
        # INPUT: Regression Epochs (Response Locked)
        "reg_epochs": pipe_out / "Method_Regression" / "Response_Locked" / "epochs",
        
        # INPUT: Baseline Covariate (Using Stimulus-Locked Pre-Stim Baseline)
        "reg_base": pipe_out / "Baseline_Raw" / "Stimulus_Locked_Values" / "baseline_values.csv",
        
        # Channel Locations (Borrowed from Stimulus-Locked Standard output)
...
def load_bl(path):
    ...
    # Force use of Stimulus-Locked Pre-Offer Baseline for Response-Locked data
...
# ==============================================================================
# 6. REGRESSION ANALYSIS MODULE (DYNAMIC E1 / E2)
# ==============================================================================
...
class RegressionAnalysis:
    ...
    def run(self):
        print(f"\n🚀 Regression Mode (Response-Locked): {TARGET_COMPONENT}")
        print(f"   -> Version Configured: {EXPERIMENT_VERSION}")
        ...
        print("   Aligning data & Mean-Centering Baseline...")
        ...
        # [CRITICAL SCIENTIFIC FIX]: Mean-center the baseline to match R LMM behavior.
    ...
    def _make_design_matrix(self, dt):
        """
        Dynamically constructs the design matrix.
        Standardized ANCOVA specification for both E1 and E2.
        """
        X_list = [np.ones(len(dt))] # 0. Intercept
        for e in ['aff','dis','dom','enj']: X_list.append(dt[f'e_{e}'].values) # 1-4. Emo
        X_list.append(dt['oc'].values) # 5. Offer
        for e in ['aff','dis','dom','enj']: X_list.append(dt[f'e_{e}'].values * dt['oc'].values) # 6-9. Emo*Offer
        X_list.append(dt['BL'].values) # 10. Baseline Main Effect
    ...
    def _generate_topomap_csv(self, df, channels):
        ...
        for ch in tqdm(channels, desc="Channel Reg"):
            ...
            # Reconstruct Design Matrix per valid slice
