...
"""
CPP Master Analysis Script (Exp 1 - Sequential) - REGRESSION ONLY
=============================================================================
Description:
    A robust analysis pipeline for the Response-Locked CPP component (Exp 1).
    
    [DIFFERENCE FROM EXP 2]:
    - Design: Sequential (Face -> Offer -> Response).
    - Baseline: Pre-Offer baseline (-200ms to 0ms) is used.
    - MODEL CHANGE: Includes 'Emotion x Baseline' interaction.
      Reason: Since the Face is presented before the Offer, the Pre-Offer 
      baseline is modulated by the Emotion. We must model this interaction 
      to correctly isolate the phasic response to the Offer/Decision.
    
    MODEL FORMULA:
    Signal ~ Intercept + Emotion + Offer + Emo*Offer + Baseline + Emo*Baseline

    [COVARIATE SOURCE]:
    - Uses 'Baseline_LPP_offer' from E1 Stimulus-Locked Output.
...
def get_paths():
    ...
    paths = {
        "results": script_path.parent / f"Results_{TARGET_COMPONENT}_E1" / f"Session_{ts}",
        
        # Input: Regression Epochs (Response Locked)
        "reg_epochs": pipe_out / "Method_Regression" / "Response_Locked" / "epochs",
        
        # Input: Baseline Covariate (Using Stimulus-Locked Pre-Offer Baseline)
...
class VisualizationEngine:
    ...
    def plot_unified_panels(data_dict, time_ms, title_suffix, save_path):
        ...
        for idx, offer in enumerate(OFFER_TYPES):
            ...
            ax.set_title(f'{TARGET_COMPONENT} ROI: {offer.capitalize()} ({title_suffix})', fontsize=16, weight='bold', pad=12)
        ...
        plt.suptitle(f"{TARGET_COMPONENT} Waveforms (Response-Locked) - Exp 1", fontsize=20, weight='bold', y=0.98)
...
def load_bl(path):
    ...
    df = pd.read_csv(path)
    # Uses E1 Stimulus-Locked Baseline (Pre-Offer)
...
# ==============================================================================
# 5. REGRESSION ANALYSIS MODULE (EXP 1 ADAPTED)
# ==============================================================================
...
class RegressionAnalysis:
    ...
    def run(self):
        print(f"\n🚀 Regression Mode (Exp 1 - Sequential): {TARGET_COMPONENT}")
        print("   -> Model: Signal ~ Emo + Offer + Emo*Offer + Baseline + Emo*Baseline")
        ...
        self.viz.plot_unified_panels(
            ...
            "Regression-Corrected (Exp 1 Interaction Model)", 
    ...
    def _generate_topomap_csv(self, df, channels):
        ...
        for ch in tqdm(channels, desc="Channel Reg"):
            ...
            try:
                ...
                # Note: Interaction terms (Emo*BL) also become 0 when BL=0.
                # So the formula for 'Raw' values looks similar, but 'b' indices shift.
                def get_val(offer, emo):
                    v = b[0] # Intercept
                    ov = -1 if offer == 'fair' else 1
                    ...
                    v += b[5]*ov # Offer Main
                    # b[10] is Baseline Main (ignored/set to 0)
                    # b[11-14] is Emo*Baseline (ignored/set to 0)
                    return v
                
                # ... [Rest of Topomap calc is standard] ...
                raw_vals = {}
                for ofr in OFFER_TYPES:
                    for emo in COLORS:
                        v = get_val(ofr, emo)
                        row[f'raw_{emo}_{ofr}'] = v
                        raw_vals[f'{emo}_{ofr}'] = v
                rows.append(row)
            except: pass
    ...
    def _make_design_matrix(self, dt):
        """
        [EXP 1 MODEL - SEQUENTIAL]
        Structure of X:
        0: Intercept
        1-4: Emotion (Aff, Dis, Dom, Enj)
        5: Offer
        6-9: Emo * Offer
        10: Baseline Main Effect
        11-14: Emo * Baseline (INTERACTION ADDED)
        """
        X_list = [np.ones(len(dt))] # 0. Intercept
        
        # 1-4. Emotion Main
        for e in ['aff','dis','dom','enj']: 
            X_list.append(dt[f'e_{e}'].values)
        ...
        # 6-9. Emo * Offer
        for e in ['aff','dis','dom','enj']:
            X_list.append(dt[f'e_{e}'].values * dt['oc'].values)
            
        # 10. Baseline Main Effect
        X_list.append(dt['BL'].values)
        
        # 11-14. Emo * Baseline (NEW FOR E1)
        for e in ['aff','dis','dom','enj']:
             X_list.append(dt[f'e_{e}'].values * dt['BL'].values)
...
if __name__ == "__main__":
    try:
        print(f"=== PROCESSING COMPONENT (Exp 1): {TARGET_COMPONENT} ===")
        print(f"Mode: Regression Only (Interaction Model)")
