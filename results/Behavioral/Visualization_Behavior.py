...
"""
Sta_Behaviour Downstream Visualization Pipeline (v1.7 - Dynamic E1/E2 Routing)
================================================================================
Description:
    ...
    APA-compliant R pipeline. Now features dynamic globbing to automatically 
    detect E1/E2 versioned folders and files.

    INPUTS (Auto-detected from project root):
    1) R Outputs (Excel):
       results/Behavioral/*_(Pub_Output_Final|Debug_Output_Final)/<LABEL>/STATS_REPORT_*.xlsx
    ...
    OUTPUTS:
      results/Behavioral/*_(Pub_Output_Final|Debug_Output_Final)/Figures/
...
# ==============================================================================
# 2) Utilities: path detection (UPDATED FOR DYNAMIC E1/E2 ROUTING)
# ==============================================================================
...
def choose_output_root(project_root: Path, prefer_debug: bool = False) -> Path:
    """
    Dynamically find the Behavioral output directory regardless of E1/E2 prefix.
    """
    ...
    # Use glob to find directories ending with the suffix
    pub_dirs = list(beh_root.glob("*_Pub_Output_Final"))
    dbg_dirs = list(beh_root.glob("*_Debug_Output_Final"))

    if prefer_debug and dbg_dirs:
        return dbg_dirs[0]
    if pub_dirs:
        return pub_dirs[0]
    if dbg_dirs:
    ...
    raise FileNotFoundError(
        f"No output root (*_Pub_Output_Final or *_Debug_Output_Final) found in {beh_root}"
    )
...
# ==============================================================================
# 8) Main execution per label folder (UPDATED FOR DYNAMIC EXCEL GLOBBING)
# ==============================================================================

def process_label_folder(label_dir: Path, figures_dir: Path):
    print(f"\n>>> Processing Label: {label_dir.name}")
    
    # [UPDATED]: Dynamically find the Excel file, ignoring the E1/E2 prefix
    excel_candidates = list(label_dir.glob("STATS_REPORT_*.xlsx"))
    if not excel_candidates:
        print(f"   [Skip] Expected Excel file not found in: {label_dir.name}")
        return
