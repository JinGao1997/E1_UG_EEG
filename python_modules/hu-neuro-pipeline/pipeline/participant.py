...
# -------------------------------------------------------
    # [新增] 智能缺失检测与日志报告 (Smart Missing Trigger Log)
    # -------------------------------------------------------
    # 只有当用户显式允许忽略错误时 (ignore/warn)，才执行此检查
    if epochs_on_missing in ['ignore', 'warn']:
        # 1. 获取数据中实际存在的 Trigger 代码 (第三列是 Event Code)
        actual_triggers = set(events[:, 2])
        
        # 2. 获取我们请求的 Trigger 代码
        # event_id 是一个字典 {'Label': Code}，我们需要 values
        requested_triggers = set(event_id.values())
        
        # 3. 计算差集：请求了但没找到的
        missing_triggers = requested_triggers - actual_triggers
        
        if missing_triggers:
            # 反查 Label 名字方便阅读
            missing_labels = [k for k, v in event_id.items() if v in missing_triggers]
            print(f"\n[INFO] ⚠️  MISSING TRIGGERS DETECTED:")
            print(f"        Codes : {list(missing_triggers)}")
            print(f"        Labels: {missing_labels}")
            print(f"        (Action: {epochs_on_missing.upper()} - Pipeline will continue)\n")
    # -------------------------------------------------------
    # Epoching including baseline correction
    if baseline is not None:
        baseline = tuple(baseline)

    # [关键步骤] 使用参数 epochs_on_missing 创建 Epochs
    epochs = Epochs(filt, events, event_id, epochs_tmin, epochs_tmax, baseline,
                    preload=True, on_missing=epochs_on_missing)

    # 只有在允许忽略缺失时才执行，防止 get_bad_channels 崩溃
    if epochs_on_missing == 'ignore':
        actual_codes = set(epochs.events[:, 2])
        old_keys = set(epochs.event_id.keys())
        # 重建 event_id，只保留有数据的 Trigger
        epochs.event_id = {k: v for k, v in epochs.event_id.items() if v in actual_codes}
        
        removed = old_keys - set(epochs.event_id.keys())
        if removed:
            print(f"Stats: Cleaned up empty triggers from event_id: {removed}")
